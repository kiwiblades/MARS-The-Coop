...
export const registerNotificationHandlers = (io, socket) => {
    ...
    // // when a new msg is sent, to update unread counts for other members
    // socket.on('notify_new_message', async ({ chatId, senderId }) => {
    //     try {
    //         const updatedMembers = await incrementUnread();
    //         // notify each affected user's client to update their unread badge
    ...
    //         }
    //     } catch(e) {
    //         console.error('[notifications] error incrementing unread:', e.message);
    //         socket.emit('notification_error', { message: e.message });
    //     }
    // });
